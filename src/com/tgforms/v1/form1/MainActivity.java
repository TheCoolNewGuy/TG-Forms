...
import com.tgforms.v1.connectivity.SocketClient;
...
//	    intentFilter.addAction(WifiP2pManager.WIFI_P2P_THIS_DEVICE_CHANGED_ACTION);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
	    ...
	    case R.id.menubtnsave:
	    	if(Utilities.isFormOneComplete() && storedata.workCompleteSignDone)
	    		...
	    		Utilities.showToast(getApplicationContext(), "Please complete all required fields");
        	return true;
	...
	private class SendMessage extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				client = new Socket("10.0.3.15", 4444); // connect to the server
	...
	private TransferableObject getTransferableData(){
		...
		for(int i=0; i<10;i++){
		    ...
		    TAclearanceDataList.put(String.valueOf(i)+1, new TransferableClearanceData(data.getName(), data.getBase64SignCleartoOpen(), data.getDateOpen(), data.getBase64SignCleartoClose(), data.getDateClose(), data.getName().length()>0));
        }
		

		return new TransferableObject(storedata.FormDate, storedata.Location, storedata.Permit_manager, storedata.Equipment, storedata.workComplete, storedata.lotoSteps, TAclearanceDataList, "");
	}
